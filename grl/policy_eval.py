...
@partial(jit, static_argnames=['gamma', 'n_states', 'n_obs'])
def analytical_pe(pi_abs: jnp.ndarray, phi: jnp.ndarray, T: jnp.ndarray,
    ...
    # MC*
    state_v, state_q = solve_mdp(pi_ground, T, R, gamma, n_states)
    state_vals = {'v': state_v, 'q': state_q}

    occupancy = get_occupancy(pi_ground, T, p0, n_states, gamma)

    p_pi_of_s_given_o = get_p_s_given_o(n_obs, phi, occupancy)
    mc_vals = solve_amdp(state_q, p_pi_of_s_given_o, pi_abs)

    # TD
    T_obs_obs, R_obs_obs = create_td_model(p_pi_of_s_given_o, phi, T, R, n_obs)
    td_v_vals, td_q_vals = solve_mdp(pi_abs, T_obs_obs, R_obs_obs, gamma, T_obs_obs.shape[-1])
    td_vals = {'v': td_v_vals, 'q': td_q_vals}

    return state_vals, mc_vals, td_vals

@partial(jit, static_argnums=[0])
def get_p_s_given_o(n_obs: int, phi: jnp.ndarray, occupancy: jnp.ndarray):
...
def get_occupancy(pi_ground: jnp.ndarray, T: jnp.ndarray, p0: jnp.ndarray,
...
def solve_mdp(pi: jnp.ndarray, T: jnp.ndarray, R: jnp.ndarray, gamma: float, n_states: int):
    """
...
@jit
def solve_amdp(mdp_q_vals: jnp.ndarray, p_pi_of_s_given_o: jnp.ndarray,
               pi_abs: jnp.ndarray):
...
def create_td_model(p_pi_of_s_given_o: jnp.ndarray, phi: jnp.ndarray,
...
class PolicyEval:
    ...
    def _solve_mdp(self, mdp, pi):
        v, q = solve_mdp(pi, mdp.T, mdp.R, mdp.gamma, mdp.n_states)
    ...
    def get_occupancy(self, pi: jnp.ndarray):
        ...
        return get_occupancy(pi_ground, self.amdp.T, self.amdp.p0,
    ...
    def _solve_amdp(self, mdp_q_vals: jnp.ndarray, p_pi_of_s_given_o: jnp.ndarray,
        ...
        """
        return solve_amdp(mdp_q_vals, p_pi_of_s_given_o, pi)

    def _create_td_model(self, p_pi_of_s_given_o: jnp.ndarray):
        T_obs_obs, R_obs_obs = create_td_model(p_pi_of_s_given_o, self.amdp.phi,
