...
def next_step_mstd_err(
    ...
    # Repeat across front again, for NEXT step TD errors
    tde_soasoasoa = (
        tde_soasoa[None, None, None, ...]
        .repeat(n_states, axis=0)
        .repeat(n_obs, axis=1)
        .repeat(n_actions, axis=2)
    )
    ...
    if error_type == 'l2':
        mag_tde_soasoasoa = (tde_soasoasoa**2)
    elif error_type == 'abs':
        mag_tde_soasoasoa = jnp.abs(tde_soasoasoa)
    else:
        raise NotImplementedError(f"Error {error_type} not implemented yet in mem_loss fn.")
    ...
    # Compute Pr(s,o,a,s',o',a',s'',o'',a'')
    pr_s_soasoasoa =            pr_s[   :, None, None, None, None, None, None, None, None] # Pr(s)
    phi_soasoasoa =           phi_so[   :,    :, None, None, None, None, None, None, None] # Pr(o|s)
    pi_soasoasoa =                pi[None,    :,    :, None, None, None, None, None, None] # Pr(a|o)
    T_soasoasoa =              T_sas[   :, None,    :,    :, None, None, None, None, None] # Pr(s'|s,a)
    next_phi_soasoasoa =      phi_so[None, None, None,    :,    :, None, None, None, None] # Pr(o'|s')
    next_pi_soasoasoa =           pi[None, None, None, None,    :,    :, None, None, None] # Pr(a'|o')
    next_T_soasoasoa =         T_sas[None, None, None,    :, None,    :,    :, None, None] # Pr(s''|s',a')
    next_next_phi_soasoasoa = phi_so[None, None, None, None, None, None,    :,    :, None] # Pr(o''|s'')
    next_next_pi_soasoasoa =      pi[None, None, None, None, None, None, None,    :,    :] # Pr(a''|o'')
    # Pr(s,o,a,s',o',a') = Pr(s) *
    # Pr(o|s) * Pr(a|o) * Pr(s'|s,a) *
    # Pr(o'|s') * Pr(a'|o') * Pr(s''|s',a') *
    # Pr(o''|s'') * Pr(a''|o'')
    pr_soasoasoa = (
            pr_s_soasoasoa *
            phi_soasoasoa * pi_soasoasoa * T_soasoasoa *
            next_phi_soasoasoa * next_pi_soasoasoa * next_T_soasoasoa *
            next_next_phi_soasoasoa * next_next_pi_soasoasoa
    )

    # Reweight squared errors according to Pr(s,o,a,s',o',a')
    weighted_sq_tde_soasoa = pr_soasoasoa * mag_tde_soasoasoa
