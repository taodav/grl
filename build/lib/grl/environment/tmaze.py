...
def fixed_four_tmaze(n: int = 0, discount: float = 0.9,
    ...
    T_up = np.zeros((n_states, n_states))
    T_down = T_up.copy()
    ...
    all_nonterminal_idxes = np.arange(n_states - 1)
    # Always go right
    T_up[all_nonterminal_idxes[:-4], all_nonterminal_idxes[4:]] = 1
    T_down[all_nonterminal_idxes[:-4], all_nonterminal_idxes[4:]] = 1
    ...
    R_up = np.zeros((n_states, n_states))
...
def tmaze_two_goals(n: int,
    ...
    n_states += 2 # Start
    n_states += 2 # Junction
    n_states += 2 # Terminal + goal states (includes both goal/non-goal)


    T_up = np.eye(n_states, n_states)
    T_down = T_up.copy()
    T_up[[GOAL, TERMINAL, JUNCTION_DOWN, JUNCTION_UP], [GOAL, TERMINAL, JUNCTION_DOWN, JUNCTION_UP]] = 0
    T_down[[GOAL, TERMINAL, JUNCTION_DOWN, JUNCTION_UP], [GOAL, TERMINAL, JUNCTION_DOWN, JUNCTION_UP]] = 0

    # If we go up or down at the junctions, we terminate
    T_up[[JUNCTION_DOWN, JUNCTION_UP], [TERMINAL, GOAL]] = 1
    T_down[[JUNCTION_DOWN, JUNCTION_UP], [GOAL, TERMINAL]] = 1
    ...
    # transition to -2 (left) or +2 (right) index
    all_nonterminal_idxes = np.arange(n_states - 2)
    T_left[all_nonterminal_idxes[2:], all_nonterminal_idxes[:-2]] = 1
    T_right[all_nonterminal_idxes[:-2], all_nonterminal_idxes[2:]] = 1
    ...
    # If rewarding state is north
    R_up[JUNCTION_UP, GOAL] = good_term_reward
    R_down[JUNCTION_UP, TERMINAL] = bad_term_reward

    # If rewarding state is south
    R_up[JUNCTION_DOWN, TERMINAL] = bad_term_reward
    R_down[JUNCTION_DOWN, GOAL] = good_term_reward
    ...
    # Observation function with have 5 possible obs:
    # start_up, start_down, corridor, junction, goal and terminal
    phi = np.zeros((n_states, 4 + 2))
