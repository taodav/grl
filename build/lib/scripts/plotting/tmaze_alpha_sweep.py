...
args_to_extract = ['spec', 'algo', 'n_mem_states', 'flip_count_prob', 'alpha', 'seed']
...
for results_path in list(results_dir.iterdir()):
    ...
    single_row.update({
        'init_policy_discrep': init_policy_info['discrep'].item(),
        'final_mem_discrep': final_mem_info['discrep'].item(),
