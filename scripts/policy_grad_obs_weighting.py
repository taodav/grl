...
if __name__ == "__main__":
    ...
    all_policy_grads = jnp.zeros((pomdp.observation_space.n, ) + pi_params.shape)
    for o in range(pomdp.observation_space.n):
        all_policy_grads = all_policy_grads.at[o].set(policy_grad_fn(pi_params, pomdp, o))

    # l2 norm of all gradients
    pg_norms = jnp.sqrt(jnp.einsum('ijk->i', all_policy_grads**2))

    outs, params_grad = jax.value_and_grad(pg_objective_func, has_aux=True)(pi_params, pomdp)
